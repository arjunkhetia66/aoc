...
remove_element_from_vector <- function(input, exclude) {
  return(input[input != exclude])
}

check_vector_uniqueness <- function(input_vec) {
  #check the uniqueness of identifying column/s
  
  original_len <- length(input_vec)
  unique_len <- length(input_vec %>% unique())
  
  return(original_len == unique_len)
